-- Additive: optional idempotency key on bookings for safe public retries.
-- Does not alter existing rows; unique only when key is present.
...
comment on column public.bookings.idempotency_key is
  'Optional client/server idempotency key to prevent duplicate bookings on retry.';

-- Extend begin_booking with optional idempotency key (default null keeps prior callers valid).
...
begin
  ...
  if p_idempotency_key is not null and length(trim(p_idempotency_key)) > 0 then
    ...
    if found then
      ...
      return jsonb_build_object(
        ...
        'total_price_gross_grosz', v_existing.total_price_gross_grosz,
        'amount_to_pay_gross_grosz', v_existing.total_price_gross_grosz,
  ...
  insert into public.payments (
    ...
    coalesce(nullif(trim(p_idempotency_key), ''), encode(gen_random_bytes(16), 'hex'))
