-- Ceramika Nero — Phase 5 follow-up fix
--
-- The pgcrypto functions used by 00000000000005_booking_system.sql are installed
-- in the `extensions` schema by default, but the Phase 5 functions use
-- `set search_path = public`. This migration recreates the affected functions
-- with explicit schema-qualified pgcrypto calls so they execute correctly at
-- runtime without widening the search path.
--
-- All changes are forward-only and additive. No data is modified.

-- ---------------------------------------------------------------------------
-- begin_booking: replace gen_random_bytes with extensions.gen_random_bytes
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- create_cancellation_token: schema-qualify pgcrypto functions
-- ---------------------------------------------------------------------------

create or replace function public.create_cancellation_token(
  p_booking_id uuid,
...
comment on function public.create_cancellation_token(uuid, timestamptz) is
  'Generates a secure, hashed cancellation token, stores the hash and returns the plaintext.';

-- ---------------------------------------------------------------------------
-- verify_cancellation_token: schema-qualify pgcrypto digest
-- ---------------------------------------------------------------------------

create or replace function public.verify_cancellation_token(
  p_booking_id uuid,
  p_token text
)
returns boolean
...
-- ---------------------------------------------------------------------------
-- Re-apply least-privilege grants after recreating the functions
-- ---------------------------------------------------------------------------
